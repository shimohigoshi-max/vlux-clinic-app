...
- **iPad View (Clinician Terminal)**: Voice recording (Web Speech API), AI-powered two-step analysis (auto-summarize + full karte generation), treatment history with AI correlation analysis, HealthKit sync simulation, Data Platform analytics, Revenue Model simulator
- **Smartphone View (Patient)**: Timeline with doctor's notes + AI life advice card (monthly theme, 5-axis focus areas with priority, today's action, next visit goal), health metrics, coupon wallet (3 types), booking button, VLUXストア with rank-based pricing, VLUXスコア membership rank system (5 tiers), ロードマップ (5-phase roadmap), cross-clinic data inheritance demo
...
- `client/src/components/ipad-view.tsx` - Clinician iPad view with 6 tabs: Voice, Karte, History/Correlation, Health, データ基盤 (Data Platform), 収益モデル (Revenue Model)
- `client/src/components/smartphone-view.tsx` - Patient smartphone view with 5 tabs: タイムライン, 健康データ, VLUXストア (rank-based pricing), VLUXスコア (membership card + roadmap), ロードマップ (5-phase roadmap). Includes ClinicBanner for cross-clinic switching.
- `client/src/lib/constants.ts` - Types, sample data, 8 treatment history records, 5-tier rank system, status color utilities, PHASES (5 phases), CLINIC_MASTER (2 clinics), revenue constants (REV_UNIT_PRICE, REV_CONV_RATE, REV_SHARE_RATE, SAAS_MONTHLY_FEE)
...
## Version History
- v2.4: 5-tier rank system, coupon wallet
- v2.5: Life Advice Card (AI-generated, 5-axis focus areas)
- v2.6: Backend validation, rate limiting
- v2.7: Roadmap tab (5 phases, expandable cards, color-coded by status)
- v2.8: Data Platform tab (network scale, 4-quadrant value map, compliance)
- v2.9: Revenue Model tab (3 sub-tabs: clinic/EC/SaaS with real-time sliders)
- v3.0: Cross-clinic data inheritance demo (clinic switcher, data continuity)
- v3.1: VLUX branding overhaul (logo, tab renames, version bump)

## Key Constants
- VISIT_COUNT = 8, current rank = Bronze
- PHASES: 5 phases (done/active/pending/final statuses)
- CLINIC_MASTER: tanaka (田中整骨院, default), sakai (堺整骨院グループ)
- Revenue: REV_UNIT_PRICE=3500, REV_CONV_RATE=0.25, REV_SHARE_RATE=0.08, SAAS_MONTHLY_FEE=50000
