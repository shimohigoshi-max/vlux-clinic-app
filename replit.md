# Connected Healthcare Ecosystem Demo v2.6
...
- **Smartphone View (Patient)**: Timeline with doctor's notes + AI life advice card (monthly theme, 5-axis focus areas with priority, today's action, next visit goal), health metrics, coupon wallet (3 types), booking button, shop with rank-based pricing, membership rank system (5 tiers)
...
- `client/src/lib/constants.ts` - Types (SummaryResult, KarteResult with LifeAdvice, CorrelationResult, TreatmentRecord, Rank), sample data, 8 treatment history records, 5-tier rank system, status color utilities
