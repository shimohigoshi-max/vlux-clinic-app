      ...
      <div style={{ background: "linear-gradient(90deg,#060f20,#0b1e3a)", borderBottom: "1px solid #102840", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 9, color: "#2a5070", letterSpacing: 3 }}>ECOSYSTEM DEMO v2.4</div>
          </div>
        </div>
            ...
            <div style={{ padding: 16, minHeight: 380 }}>
              ...
              {phoneTab === "rank" && (
                <div>
                  {/* Current rank card */}
                  {rank ? (
                    <div style={{ background: rank.gradient, border: `1px solid ${rank.border}`, borderRadius: 18, padding: 18, marginBottom: 14, position: "relative", overflow: "hidden", boxShadow: `0 4px 24px ${rank.glow}30` }}>
                      <div style={{ position: "absolute", top: -20, right: -20, fontSize: 80, opacity: 0.08 }}>{rank.icon}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                        <div style={{ fontSize: 36 }}>{rank.icon}</div>
                        <div>
                          <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: rank.glow, letterSpacing: 3 }}>{rank.labelEn}</div>
                          <div style={{ fontSize: 20, fontWeight: 700, color: rank.color }}>{rank.label}会員</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                        {[
                          { label: "通算来院", val: `${VISIT_COUNT}回`, icon: "🏥" },
                          { label: "通販割引", val: `${rank.ecDiscount}%`, icon: "🛒" },
                          { label: "ポイント還元", val: `${rank.pointRate}%`, icon: "💰" },
                        ].map(m => (
                          <div key={m.label} style={{ flex: 1, background: "rgba(0,0,0,.3)", borderRadius: 10, padding: "8px 6px", textAlign: "center" }}>
                            <div style={{ fontSize: 14 }}>{m.icon}</div>
                            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 13, color: rank.color, fontWeight: 700 }}>{m.val}</div>
                            <div style={{ fontSize: 8, color: rank.glow, marginTop: 1 }}>{m.label}</div>
                          </div>
                        ))}
                      </div>
                      ...
                      {nextRank ? (
                        <div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: rank.glow, marginBottom: 4 }}>
                            <span>次のランク：{nextRank.label}（{nextRank.visits}回）</span>
                            <span>{VISIT_COUNT} / {nextRank.visits}回</span>
                          </div>
                          <div style={{ height: 6, background: "rgba(0,0,0,.4)", borderRadius: 3 }}>
                            <div style={{ width: `${(VISIT_COUNT / nextRank.visits) * 100}%`, height: "100%", background: `linear-gradient(90deg,${rank.glow},${nextRank.glow})`, borderRadius: 3, transition: "width 1s ease" }} />
                          </div>
                          <div style={{ fontSize: 9, color: rank.glow, marginTop: 4, textAlign: "right" }}>あと {nextRank.visits - VISIT_COUNT} 回で {nextRank.icon} {nextRank.label}へ</div>
                        </div>
                      ) : (
                        <div style={{ textAlign: "center", fontSize: 10, color: rank.glow, padding: "6px 0" }}>✨ 最高ランク達成！すべての特典が解放されています</div>
                      )}
                    </div>
                  ) : (
                    <div style={{ textAlign: "center", padding: 30, color: "#2a5070" }}>
                      <div style={{ fontSize: 30, marginBottom: 8 }}>🥉</div>
                      <div style={{ fontSize: 12 }}>5回来院でブロンズランク獲得！</div>
                    </div>
                  )}

                  {/* Current rank perks */}
                  {rank && (
                    <div style={{ background: "rgba(255,255,255,.03)", border: `1px solid ${rank.border}40`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
                      <div style={{ fontSize: 10, color: rank.glow, letterSpacing: 2, marginBottom: 10 }}>現在の特典一覧</div>
                      {rank.perks.map((p, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 7 }}>
                          <span style={{ color: rank.glow, fontSize: 12, marginTop: 1 }}>✓</span>
                          <span style={{ fontSize: 12, color: "#a0c0d8", lineHeight: 1.5 }}>{p}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {/* All ranks ladder */}
                  <div style={{ fontSize: 10, color: "#2a5070", letterSpacing: 2, marginBottom: 10 }}>ランクロードマップ</div>
                  {[...RANKS].reverse().map((r, i) => {
                    const achieved = VISIT_COUNT >= r.visits;
                    const isCurrent = rank?.id === r.id;
                    return (
                      <div key={r.id} style={{ display: "flex", gap: 10, marginBottom: 8, opacity: achieved ? 1 : 0.45 }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 20 }}>
                          <div style={{ width: 10, height: 10, borderRadius: "50%", background: achieved ? r.glow : "#1a3a5a", marginTop: 4, boxShadow: isCurrent ? `0 0 6px ${r.glow}` : "none" }} />
                          {i < RANKS.length - 1 && <div style={{ width: 1, flex: 1, background: "#102840", marginTop: 2 }} />}
                        </div>
                        <div style={{ flex: 1, background: isCurrent ? r.gradient : "rgba(255,255,255,.02)", border: `1px solid ${isCurrent ? r.border : "#0e2438"}`, borderRadius: 10, padding: "10px 12px", boxShadow: isCurrent ? `0 0 12px ${r.glow}25` : "none" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                            <span style={{ fontSize: 16 }}>{r.icon}</span>
                            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: isCurrent ? r.color : "#4a6a80", fontWeight: 700 }}>{r.label}</div>
                            {isCurrent && <div style={{ marginLeft: "auto", fontSize: 8, background: `${r.glow}30`, border: `1px solid ${r.glow}60`, borderRadius: 4, padding: "1px 7px", color: r.glow }}>現在</div>}
                            <div style={{ marginLeft: isCurrent ? 0 : "auto", fontSize: 9, color: "#2a5070" }}>{r.visits}回〜</div>
                          </div>
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {[`割引 ${r.couponDiscount.toLocaleString()}円/回`, `通販 ${r.ecDiscount}%OFF`, `PT ${r.pointRate}%`].map(t => (
                              <div key={t} style={{ fontSize: 9, background: "rgba(0,0,0,.3)", borderRadius: 4, padding: "2px 7px", color: isCurrent ? r.glow : "#2a4a60" }}>{t}</div>
                            ))}
                          </div>
                        </div>
                      </div>
