...
const RANKS = [
  { id: "eternal_platinum", label: "永久プラチナ", labelEn: "ETERNAL PLATINUM", visits: 80, icon: "💎", color: "#e0f0ff", glow: "#80c8ff", gradient: "linear-gradient(135deg,#0a2a50,#061828,#0a3560)", border: "#80c8ff", couponDiscount: 2000, pointRate: 10, ecDiscount: 20, referralBonus: 2000, perks: ["施術¥2,000 OFF（毎回）","通販20%OFF","ポイント10%還元","紹介ボーナス¥2,000","専任担当制","優先予約枠","特別イベント招待"] },
  { id: "platinum",         label: "プラチナ",     labelEn: "PLATINUM",         visits: 50, icon: "🌟", color: "#f0e8ff", glow: "#c090ff", gradient: "linear-gradient(135deg,#2a1a4a,#160a2a,#3a1a5a)", border: "#c090ff", couponDiscount: 1500, pointRate: 7,  ecDiscount: 15, referralBonus: 1500, perks: ["施術¥1,500 OFF（毎回）","通販15%OFF","ポイント7%還元","紹介ボーナス¥1,500","優先予約枠","バースデークーポン"] },
  { id: "gold",             label: "ゴールド",     labelEn: "GOLD",             visits: 30, icon: "🥇", color: "#fff8e0", glow: "#ffd060", gradient: "linear-gradient(135deg,#3a2a00,#201800,#3a3000)",  border: "#d4a030", couponDiscount: 1000, pointRate: 5,  ecDiscount: 10, referralBonus: 1000, perks: ["施術¥1,000 OFF（毎回）","通販10%OFF","ポイント5%還元","紹介ボーナス¥1,000","先行商品案内"] },
  { id: "silver",           label: "シルバー",     labelEn: "SILVER",           visits: 15, icon: "🥈", color: "#e8eff8", glow: "#90a8c8", gradient: "linear-gradient(135deg,#1a2030,#0e1520,#202838)", border: "#7090b8", couponDiscount: 700,  pointRate: 3,  ecDiscount: 7,  referralBonus: 700,  perks: ["施術¥700 OFF（毎回）","通販7%OFF","ポイント3%還元","紹介ボーナス¥700"] },
  { id: "bronze",           label: "ブロンズ",     labelEn: "BRONZE",           visits: 5,  icon: "🥉", color: "#f8ede0", glow: "#c08060", gradient: "linear-gradient(135deg,#2a1a0a,#180e06,#2a2010)",  border: "#a07040", couponDiscount: 500,  pointRate: 1,  ecDiscount: 3,  referralBonus: 500,  perks: ["施術¥500 OFF（毎回）","通販3%OFF","ポイント1%還元","紹介ボーナス¥500"] },
];
const getRank    = (v) => RANKS.find(r => v >= r.visits) || null;
const getNextRank = (v) => { const i = RANKS.findIndex(r => v >= r.visits); return i > 0 ? RANKS[i - 1] : null; };
...
export default function App() {
  ...
  const doKarte = async () => {
    ...
    const prevHistory = TREATMENT_HISTORY.slice(0, 3).map(h => `${h.date} ${h.area} 疼痛${h.pain}/10`).join(" / ");
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          system: `整骨院AIカルテ。JSONのみ出力（\`\`\`不要）:
{
  ...
  "life_advice": {
    ...
    "focus_areas": [
      {"category": "睡眠", "icon": "😴", "advice": "具体的なアドバイス1文", "priority": "高/中/低"},
      {"category": "運動", "icon": "🏃", "advice": "具体的なアドバイス1文", "priority": "高/中/低"},
      {"category": "食事・水分", "icon": "🥗", "advice": "具体的なアドバイス1文", "priority": "高/中/低"},
      {"category": "仕事環境", "icon": "💼", "advice": "具体的なアドバイス1文", "priority": "高/中/低"},
      {"category": "メンタル", "icon": "🧘", "advice": "具体的なアドバイス1文", "priority": "高/中/低"}
    ],
          ...
          system: `あなたは整骨院の臨床データアナリストです。患者の過去の治療履歴と生活習慣データ（HealthKit）を分析し、相関性・パターン・リスク予測をJSONのみで出力してください（\`\`\`不要）:
{
  ...
  const VISIT_COUNT = TREATMENT_HISTORY.length; // 8回

  // ── Health Contribution Score (6軸) ──────────────────────────────────────
  const SCORE_AXES = [
    { key: "visit",    label: "来院頻度",       weight: 0.40, icon: "🏥", value: 72, desc: "8回来院・平均2週間間隔" },
    { key: "data",     label: "連携データ蓄積", weight: 0.30, icon: "📊", value: 58, desc: "HealthKit 44日連携・4指標" },
    { key: "purchase", label: "EC購入頻度",     weight: 0.10, icon: "🛒", value: 40, desc: "2回購入・直近30日1回" },
    { key: "talk",     label: "会話テキスト量", weight: 0.10, icon: "💬", value: 65, desc: "平均会話 820文字/回" },
    { key: "login",    label: "ログイン継続",   weight: 0.05, icon: "📱", value: 80, desc: "連続ログイン 12日" },
    { key: "referral", label: "紹介実績",       weight: 0.05, icon: "👥", value: 20, desc: "紹介 1名" },
  ];
  const HEALTH_SCORE = Math.round(SCORE_AXES.reduce((sum, a) => sum + a.value * a.weight, 0));

  // スコアからランクを決定（来院+データ複合）
  const SCORE_RANK_THRESHOLDS = [
    { score: 80, rankId: "eternal_platinum" },
    { score: 65, rankId: "platinum" },
    { score: 50, rankId: "gold" },
    { score: 35, rankId: "silver" },
    { score: 20, rankId: "bronze" },
  ];
  const scoreRank = SCORE_RANK_THRESHOLDS.find(t => HEALTH_SCORE >= t.score);
  const rank = scoreRank ? RANKS.find(r => r.id === scoreRank.rankId) : getRank(VISIT_COUNT);
  const nextScoreThreshold = SCORE_RANK_THRESHOLDS[SCORE_RANK_THRESHOLDS.findIndex(t => HEALTH_SCORE >= t.score) - 1];
  const nextRank = nextScoreThreshold ? RANKS.find(r => r.id === nextScoreThreshold.rankId) : null;
      ...
      <div style={{ background: "linear-gradient(90deg,#060f20,#0b1e3a)", borderBottom: "1px solid #102840", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 9, color: "#2a5070", letterSpacing: 3 }}>ECOSYSTEM DEMO v2.6</div>
          </div>
        </div>
          ...
          <div style={{ background: "#0a1628", borderRadius: 40, border: "2px solid #102840", overflow: "hidden", boxShadow: "0 30px 70px rgba(0,0,0,.7)" }}>
            ...
            <div style={{ background: "linear-gradient(160deg,#0a1e3a,#050f1e)", padding: "14px 18px 0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>田中 大輔 さん</div>
                  <div style={{ fontSize: 10, color: "#2a5070" }}>次回: 3月16日（土）14:00</div>
                </div>
                {rank && (
                  <div style={{ background: rank.gradient, border: `1px solid ${rank.border}`, borderRadius: 10, padding: "5px 10px", textAlign: "center", boxShadow: `0 0 10px ${rank.glow}40` }}>
                    <div style={{ fontSize: 16 }}>{rank.icon}</div>
                    <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 8, color: rank.color, letterSpacing: 1, fontWeight: 700 }}>{rank.label}</div>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", borderBottom: "1px solid #102840" }}>
                {[["timeline", "タイムライン"], ["health", "健康データ"], ["shop", "ショップ"], ["rank", "👑 ランク"]].map(([t, l]) => (
            ...
            <div style={{ padding: 16, minHeight: 380 }}>
                  ...
                  {patientSent && karte && !karte.error ? (
                    <div>
                      ...
                      {karte.life_advice && (
                        <div style={{ marginBottom: 10 }}>
                          {/* ヘッダー */}
                          <div style={{ background: "linear-gradient(135deg,rgba(0,180,120,.12),rgba(0,80,60,.08))", border: "1px solid #00b87840", borderRadius: 16, padding: 14, marginBottom: 8, position: "relative", overflow: "hidden" }}>
                            <div style={{ position: "absolute", top: -15, right: -15, fontSize: 60, opacity: 0.05 }}>🌱</div>

                            {/* テーマ＋更新バッジ */}
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                              <div style={{ background: "rgba(0,200,150,.15)", border: "1px solid #00c89650", borderRadius: 8, padding: "4px 10px" }}>
                                <div style={{ fontSize: 8, color: "#00a870", letterSpacing: 2, marginBottom: 1 }}>今月のテーマ</div>
                                <div style={{ fontSize: 13, color: "#00c896", fontWeight: 700 }}>{karte.life_advice.this_month_theme}</div>
                              </div>
                              <div style={{ marginLeft: "auto", fontSize: 8, background: "rgba(0,200,150,.1)", border: "1px solid #00c89640", borderRadius: 6, padding: "3px 8px", color: "#00a870" }}>✦ 今日更新</div>
                            </div>

                            {/* 前回からの改善 */}
                            {karte.life_advice.improved_from_last && (
                              <div style={{ background: "rgba(0,0,0,.2)", borderRadius: 10, padding: "8px 12px", marginBottom: 12, display: "flex", gap: 8, alignItems: "flex-start" }}>
                                <span style={{ fontSize: 14, marginTop: 1 }}>📈</span>
                                <div>
                                  <div style={{ fontSize: 8, color: "#00a870", letterSpacing: 1, marginBottom: 2 }}>前回からの改善</div>
                                  <div style={{ fontSize: 11, color: "#60a888", lineHeight: 1.6 }}>{karte.life_advice.improved_from_last}</div>
                                </div>
                              </div>
                            )}

                            {/* フォーカスエリア5軸 */}
                            <div style={{ fontSize: 8, color: "#008860", letterSpacing: 2, marginBottom: 8 }}>生活改善フォーカス</div>
                            {karte.life_advice.focus_areas?.map((area, i) => (
                              <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-start" }}>
                                <div style={{ minWidth: 28, height: 28, borderRadius: 8, background: area.priority === "高" ? "rgba(255,80,80,.15)" : area.priority === "中" ? "rgba(255,170,0,.12)" : "rgba(0,200,150,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>
                                  {area.icon}
                                </div>
                                <div style={{ flex: 1 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 2 }}>
                                    <span style={{ fontSize: 10, color: "#60a080", fontWeight: 600 }}>{area.category}</span>
                                    <span style={{ fontSize: 8, padding: "1px 5px", borderRadius: 3, background: area.priority === "高" ? "rgba(255,80,80,.2)" : area.priority === "中" ? "rgba(255,170,0,.2)" : "rgba(0,200,150,.15)", color: area.priority === "高" ? "#ff7070" : area.priority === "中" ? "#ffaa44" : "#00c896" }}>
                                      {area.priority}
                                    </span>
                                  </div>
                                  <div style={{ fontSize: 11, color: "#5a8878", lineHeight: 1.6 }}>{area.advice}</div>
                                </div>
                              </div>
                            ))}
                          </div>
                          ...
                          {/* 今日からできること */}
                          {karte.life_advice.one_thing_today && (
                            <div style={{ background: "linear-gradient(135deg,rgba(0,120,220,.1),rgba(0,60,140,.07))", border: "1px solid #0066cc40", borderRadius: 12, padding: 12, marginBottom: 8 }}>
                              <div style={{ fontSize: 8, color: "#4488cc", letterSpacing: 2, marginBottom: 5 }}>⚡ 今日からできること</div>
                              <div style={{ fontSize: 13, color: "#80b0e0", fontWeight: 600, lineHeight: 1.6 }}>{karte.life_advice.one_thing_today}</div>
                            </div>
                          )}

                          {/* 次回来院までの目標 */}
                          {karte.life_advice.next_visit_goal && (
                            <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid #0e2438", borderRadius: 12, padding: 12 }}>
                              <div style={{ fontSize: 8, color: "#2a5070", letterSpacing: 2, marginBottom: 5 }}>🎯 次回来院までの目標</div>
                              <div style={{ fontSize: 11, color: "#5080a0", lineHeight: 1.6, fontStyle: "italic" }}>"{karte.life_advice.next_visit_goal}"</div>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div style={{ marginBottom: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                          <div style={{ fontSize: 11, color: "#d4a040", fontWeight: 700 }}>🎟 クーポンウォレット</div>
                          <div style={{ fontSize: 10, color: "#4a6070" }}>通算 8回来院 · 有効 3枚</div>
                        </div>

                        {/* 今回の施術クーポン（毎回付与） */}
                        <div style={{ background: "linear-gradient(135deg,rgba(255,80,120,.13),rgba(160,30,70,.08))", border: "1px dashed #ff406070", borderRadius: 14, padding: 14, marginBottom: 8, position: "relative", overflow: "hidden" }}>
                          <div style={{ position: "absolute", top: -10, right: -10, fontSize: 50, opacity: 0.06 }}>🎫</div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            <span style={{ fontSize: 13 }}>🎫</span>
                            <div style={{ fontSize: 11, color: "#ff7090", fontWeight: 700 }}>今回の施術 来院クーポン</div>
                            <div style={{ marginLeft: "auto", background: "rgba(255,80,120,.2)", border: "1px solid #ff406060", borderRadius: 6, padding: "2px 8px", fontSize: 9, color: "#ff7090" }}>NEW · 3/31まで</div>
                          </div>
                          <div style={{ fontSize: 22, fontWeight: 700, color: "#ff9090", fontFamily: "'Space Mono',monospace", marginBottom: 3 }}>¥500 OFF</div>
                          <div style={{ fontSize: 10, color: "#703040", marginBottom: 10 }}>次回施術料金より。毎回の来院でプレゼント。</div>
                          <div style={{ background: "rgba(0,0,0,.4)", borderRadius: 8, padding: "7px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 13, color: "#ff7090", letterSpacing: 4 }}>CHE-0309</div>
                            <div style={{ fontSize: 9, color: "#402030" }}>来院時に提示</div>
                          </div>
                        </div>

                        {/* 継続通院ポイントクーポン（8回達成） */}
                        <div style={{ background: "linear-gradient(135deg,rgba(255,180,0,.1),rgba(200,120,0,.07))", border: "1px dashed #d4901060", borderRadius: 14, padding: 14, marginBottom: 8, position: "relative", overflow: "hidden" }}>
                          <div style={{ position: "absolute", top: -10, right: -10, fontSize: 50, opacity: 0.06 }}>🏆</div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            <span style={{ fontSize: 13 }}>🏆</span>
                            <div style={{ fontSize: 11, color: "#d4a040", fontWeight: 700 }}>継続通院 8回達成クーポン</div>
                            <div style={{ marginLeft: "auto", background: "rgba(212,160,64,.2)", border: "1px solid #d4a04060", borderRadius: 6, padding: "2px 8px", fontSize: 9, color: "#d4a040" }}>4/30まで</div>
                          </div>
                          <div style={{ fontSize: 22, fontWeight: 700, color: "#d4b060", fontFamily: "'Space Mono',monospace", marginBottom: 3 }}>¥1,000 OFF</div>
                          <div style={{ fontSize: 10, color: "#705030", marginBottom: 10 }}>10回達成ごとに自動付与。次は10回目で¥1,500 OFFへ。</div>
                          {/* Progress to next milestone */}
                          <div style={{ marginBottom: 10 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#604020", marginBottom: 4 }}>
                              <span>次のマイルストーン：10回</span><span>8 / 10回</span>
                            </div>
                            <div style={{ height: 5, background: "rgba(0,0,0,.3)", borderRadius: 3 }}>
                              <div style={{ width: "80%", height: "100%", background: "linear-gradient(90deg,#d4a040,#ffd060)", borderRadius: 3 }} />
                            </div>
                          </div>
                          <div style={{ background: "rgba(0,0,0,.4)", borderRadius: 8, padding: "7px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 13, color: "#d4a040", letterSpacing: 4 }}>VIP-0008</div>
                            <div style={{ fontSize: 9, color: "#503010" }}>来院時に提示</div>
                          </div>
                        </div>

                        {/* 紹介クーポン */}
                        <div style={{ background: "linear-gradient(135deg,rgba(0,200,150,.1),rgba(0,120,80,.07))", border: "1px dashed #00c89660", borderRadius: 14, padding: 14, position: "relative", overflow: "hidden" }}>
                          <div style={{ position: "absolute", top: -10, right: -10, fontSize: 50, opacity: 0.06 }}>👥</div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            <span style={{ fontSize: 13 }}>👥</span>
                            <div style={{ fontSize: 11, color: "#00c896", fontWeight: 700 }}>お友達紹介クーポン</div>
                            <div style={{ marginLeft: "auto", background: "rgba(0,200,150,.15)", border: "1px solid #00c89650", borderRadius: 6, padding: "2px 8px", fontSize: 9, color: "#00c896" }}>期限なし</div>
                          </div>
                          <div style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                            <div style={{ flex: 1, background: "rgba(0,0,0,.3)", borderRadius: 8, padding: 8, textAlign: "center" }}>
                              <div style={{ fontSize: 9, color: "#006040", marginBottom: 2 }}>紹介した方に</div>
                              <div style={{ fontSize: 16, fontWeight: 700, color: "#00c896", fontFamily: "'Space Mono',monospace" }}>¥500</div>
                              <div style={{ fontSize: 9, color: "#006040" }}>初回割引</div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", color: "#006040", fontSize: 16 }}>＋</div>
                            <div style={{ flex: 1, background: "rgba(0,0,0,.3)", borderRadius: 8, padding: 8, textAlign: "center" }}>
                              <div style={{ fontSize: 9, color: "#006040", marginBottom: 2 }}>あなたにも</div>
                              <div style={{ fontSize: 16, fontWeight: 700, color: "#00c896", fontFamily: "'Space Mono',monospace" }}>¥500</div>
                              <div style={{ fontSize: 9, color: "#006040" }}>次回割引</div>
                            </div>
                          </div>
                          <button onClick={() => alert("デモ：LINEシェア / QRコード表示")}
                            style={{ width: "100%", padding: "9px 0", background: "rgba(0,200,150,.15)", border: "1px solid #00c89650", borderRadius: 10, color: "#00c896", fontSize: 12, cursor: "pointer", fontWeight: 700 }}>
                            🔗 紹介リンクをシェアする
                          </button>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        ...
                        {/* 施術クーポン */}
                        <div style={{ background: "linear-gradient(135deg,rgba(255,80,120,.13),rgba(160,30,70,.08))", border: "1px dashed #ff406070", borderRadius: 14, padding: 12, marginBottom: 8, position: "relative", overflow: "hidden" }}>
                          <div style={{ position: "absolute", top: -8, right: -8, fontSize: 44, opacity: 0.06 }}>🎫</div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                            <span style={{ fontSize: 12 }}>🎫</span>
                            <div style={{ fontSize: 11, color: "#ff7090", fontWeight: 700 }}>来院クーポン（毎回付与）</div>
                            <div style={{ marginLeft: "auto", fontSize: 9, color: "#ff7090", background: "rgba(255,80,120,.15)", borderRadius: 4, padding: "2px 7px" }}>3/31まで</div>
                          </div>
                          <div style={{ fontSize: 20, fontWeight: 700, color: "#ff9090", fontFamily: "'Space Mono',monospace", marginBottom: 8 }}>¥500 OFF</div>
                          <div style={{ background: "rgba(0,0,0,.4)", borderRadius: 8, padding: "6px 12px", fontFamily: "'Space Mono',monospace", fontSize: 13, color: "#ff7090", letterSpacing: 4 }}>CHE-0309</div>
                        </div>

                        {/* 継続クーポン */}
                        <div style={{ background: "linear-gradient(135deg,rgba(255,180,0,.1),rgba(200,120,0,.07))", border: "1px dashed #d4901060", borderRadius: 14, padding: 12, marginBottom: 8 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                            <span style={{ fontSize: 12 }}>🏆</span>
                            <div style={{ fontSize: 11, color: "#d4a040", fontWeight: 700 }}>継続通院 8回達成クーポン</div>
                            <div style={{ marginLeft: "auto", fontSize: 9, color: "#d4a040" }}>4/30まで</div>
                          </div>
                          <div style={{ fontSize: 20, fontWeight: 700, color: "#d4b060", fontFamily: "'Space Mono',monospace", marginBottom: 6 }}>¥1,000 OFF</div>
                          <div style={{ marginBottom: 6 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#604020", marginBottom: 3 }}>
                              <span>次：10回で¥1,500 OFF</span><span>8 / 10回</span>
                            </div>
                            <div style={{ height: 4, background: "rgba(0,0,0,.3)", borderRadius: 2 }}>
                              <div style={{ width: "80%", height: "100%", background: "linear-gradient(90deg,#d4a040,#ffd060)", borderRadius: 2 }} />
                            </div>
                          </div>
                          <div style={{ background: "rgba(0,0,0,.4)", borderRadius: 8, padding: "6px 12px", fontFamily: "'Space Mono',monospace", fontSize: 13, color: "#d4a040", letterSpacing: 4 }}>VIP-0008</div>
                        </div>

                        {/* 紹介クーポン */}
                        <div style={{ background: "linear-gradient(135deg,rgba(0,200,150,.1),rgba(0,120,80,.07))", border: "1px dashed #00c89660", borderRadius: 14, padding: 12 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                            <span style={{ fontSize: 12 }}>👥</span>
                            <div style={{ fontSize: 11, color: "#00c896", fontWeight: 700 }}>お友達紹介クーポン</div>
                            <div style={{ marginLeft: "auto", fontSize: 9, color: "#00c896" }}>期限なし</div>
                          </div>
                          <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                            <div style={{ flex: 1, background: "rgba(0,0,0,.3)", borderRadius: 8, padding: "6px 8px", textAlign: "center" }}>
                              <div style={{ fontSize: 9, color: "#006040" }}>紹介した方</div>
                              <div style={{ fontSize: 15, fontWeight: 700, color: "#00c896", fontFamily: "'Space Mono',monospace" }}>¥500</div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", color: "#006040" }}>＋</div>
                            <div style={{ flex: 1, background: "rgba(0,0,0,.3)", borderRadius: 8, padding: "6px 8px", textAlign: "center" }}>
                              <div style={{ fontSize: 9, color: "#006040" }}>あなたにも</div>
                              <div style={{ fontSize: 15, fontWeight: 700, color: "#00c896", fontFamily: "'Space Mono',monospace" }}>¥500</div>
                            </div>
                          </div>
                          <button onClick={() => alert("デモ：紹介リンク / QRコード")}
                            style={{ width: "100%", padding: "8px 0", background: "rgba(0,200,150,.15)", border: "1px solid #00c89650", borderRadius: 10, color: "#00c896", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>
                            🔗 紹介リンクをシェアする
                          </button>
                        </div>
                      </div>
              ...
              {phoneTab === "shop" && (
                <div>
                  ...
                  {rank && (
                    <div style={{ background: rank.gradient, border: `1px solid ${rank.border}50`, borderRadius: 10, padding: "8px 12px", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 16 }}>{rank.icon}</span>
                      <div>
                        <div style={{ fontSize: 10, color: rank.color, fontWeight: 700 }}>{rank.label}会員特典</div>
                        <div style={{ fontSize: 11, color: rank.glow }}>通販 <span style={{ fontFamily: "'Space Mono',monospace", fontWeight: 700 }}>{rank.ecDiscount}% OFF</span> · ポイント <span style={{ fontFamily: "'Space Mono',monospace", fontWeight: 700 }}>{rank.pointRate}%</span> 還元</div>
                      </div>
                    </div>
                  )}
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
                            <div>
                              {rank && rank.ecDiscount > 0 ? (
                                <div>
                                  <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#2a5070", textDecoration: "line-through" }}>¥{p.price.toLocaleString()}</span>
                                  <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 14, color: "#00c896", marginLeft: 6, fontWeight: 700 }}>¥{discounted.toLocaleString()}</span>
                                  <div style={{ fontSize: 9, color: rank.glow, marginTop: 1 }}>+{pts}pt 獲得</div>
                                </div>
                              ) : (
                    ...
                    {DEMO_PRODUCTS.filter(p => !recProds.find(r => r.id === p.id)).map(p => {
                      ...
                      return (
                        <div key={p.id} style={{ background: "rgba(255,255,255,.02)", border: "1px solid #0e2438", borderRadius: 12, padding: 10, marginBottom: 8, display: "flex", gap: 8, alignItems: "center" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 12, color: "#7090a8" }}>{p.name}</div>
                            <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 2 }}>
                              {rank && rank.ecDiscount > 0 && <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: "#2a5070", textDecoration: "line-through" }}>¥{p.price.toLocaleString()}</span>}
                              <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: rank ? rank.glow : "#3a6070" }}>¥{discounted.toLocaleString()}</span>
                            </div>
                          </div>
                  ...
                  {cart.length > 0 && (
                    <div style={{ position: "sticky", bottom: 0, background: "#0a1628", borderTop: "1px solid #102840", paddingTop: 10, marginTop: 8 }}>
                      <button style={C.bigBtn()}>🛒 カート {cart.length}点 · ¥{cart.reduce((s, p) => { const d = rank ? Math.round(p.price*(1-rank.ecDiscount/100)) : p.price; return s+d; }, 0).toLocaleString()} — 購入する</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {phoneTab === "rank" && (
                <div>
                  ...
                  <div style={{ textAlign: "center", marginBottom: 16, position: "relative" }}>
                    ...
                    <div style={{ position: "relative", display: "inline-block" }}>
                      <svg width="160" height="160" viewBox="0 0 160 160">
                        {/* Background track */}
                        <circle cx="80" cy="80" r="66" fill="none" stroke="#0e2438" strokeWidth="10" />
                        {/* Rank segments (5 colors) */}
                        {[
                          { from: 0,  to: 20,  color: "#a07040" },
                          { from: 20, to: 35,  color: "#7090b8" },
                          { from: 35, to: 50,  color: "#d4a030" },
                          { from: 50, to: 65,  color: "#c090ff" },
                          { from: 65, to: 100, color: "#80c8ff" },
                        ].map((seg, i) => {
                          const circ = 2 * Math.PI * 66;
                          const filled = Math.max(0, Math.min(HEALTH_SCORE, seg.to) - seg.from);
                          const total = seg.to - seg.from;
                          const offset = (seg.from / 100) * circ;
                          const dash = (filled / 100) * circ;
                          const gap = circ - dash;
                          return (
                            <circle key={i} cx="80" cy="80" r="66" fill="none"
                              stroke={filled > 0 ? seg.color : "#0e2438"}
                              strokeWidth="10"
                              strokeDasharray={`${dash} ${gap}`}
                              strokeDashoffset={-(offset - circ / 4)}
                              strokeLinecap="round"
                              style={{ transition: "stroke-dasharray 1.5s ease, stroke .5s" }}
                            />
                          );
                        })}
                        {/* Rank threshold ticks */}
                        {[20, 35, 50, 65].map((thresh, i) => {
                          const angle = (thresh / 100) * 360 - 90;
                          const rad = angle * Math.PI / 180;
                          const x1 = 80 + 61 * Math.cos(rad); const y1 = 80 + 61 * Math.sin(rad);
                          const x2 = 80 + 71 * Math.cos(rad); const y2 = 80 + 71 * Math.sin(rad);
                          return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#06101e" strokeWidth="2.5" />;
                        })}
                      </svg>
                      {/* Center content */}
                      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                        <div style={{ fontSize: 10, color: "#2a5070", letterSpacing: 2 }}>HEALTH SCORE</div>
                        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 34, fontWeight: 700, color: rank ? rank.glow : "#3a6080", lineHeight: 1, marginTop: 2 }}>{HEALTH_SCORE}</div>
                        {rank && <div style={{ fontSize: 11, color: rank.color, marginTop: 4 }}>{rank.icon} {rank.label}</div>}
                      </div>
                    </div>
                    {/* Score legend */}
                    <div style={{ display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
                      {[["🥉","ブロンズ","#a07040",20],["🥈","シルバー","#7090b8",35],["🥇","ゴールド","#d4a030",50],["🌟","プラチナ","#c090ff",65],["💎","永久PT","#80c8ff",80]].map(([icon,label,color,thresh]) => (
                        <div key={label} style={{ fontSize: 8, color: HEALTH_SCORE >= thresh ? color : "#1a3a5a", display: "flex", alignItems: "center", gap: 2 }}>
                          <div style={{ width: 6, height: 6, borderRadius: "50%", background: HEALTH_SCORE >= thresh ? color : "#1a3a5a" }} />
                          {label}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* ── 6軸スコアバー ── */}
                  <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid #0e2438", borderRadius: 14, padding: 14, marginBottom: 12 }}>
                    <div style={{ fontSize: 9, color: "#2a5070", letterSpacing: 2, marginBottom: 12 }}>健康貢献スコア内訳</div>
                    {SCORE_AXES.map((ax) => (
                      <div key={ax.key} style={{ marginBottom: 10 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <span style={{ fontSize: 12 }}>{ax.icon}</span>
                            <span style={{ fontSize: 10, color: "#6090a8" }}>{ax.label}</span>
                            <span style={{ fontSize: 8, color: "#2a4060", background: "rgba(0,0,0,.3)", borderRadius: 3, padding: "1px 5px" }}>×{Math.round(ax.weight * 100)}%</span>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 9, color: "#2a5070" }}>{ax.desc}</span>
                            <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: ax.value >= 70 ? "#00c896" : ax.value >= 40 ? "#ffaa44" : "#ff6655", fontWeight: 700 }}>{ax.value}</span>
                          </div>
                        </div>
                        {/* Segmented bar */}
                        <div style={{ height: 6, background: "#0a1828", borderRadius: 3, position: "relative", overflow: "hidden" }}>
                          <div style={{
                            width: `${ax.value}%`, height: "100%", borderRadius: 3,
                            background: ax.value >= 70
                              ? "linear-gradient(90deg,#00c896,#0073e6)"
                              : ax.value >= 40
                              ? "linear-gradient(90deg,#d4a030,#ffaa44)"
                              : "linear-gradient(90deg,#a03020,#ff5540)",
                            transition: "width 1.2s ease",
                          }} />
                          {/* Contribution highlight */}
                          <div style={{ position: "absolute", top: 0, left: `${ax.value * ax.weight}%`, width: `${ax.value * ax.weight * 0.4}%`, height: "100%", background: "rgba(255,255,255,.15)", borderRadius: 3 }} />
                        </div>
                        <div style={{ textAlign: "right", fontSize: 8, color: "#1a3a5a", marginTop: 2 }}>
                          スコア貢献 <span style={{ color: "#2a5570" }}>+{Math.round(ax.value * ax.weight)}</span>
                        </div>
                      </div>
                    ))}
                    <div style={{ borderTop: "1px solid #0e2438", marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
                      <div style={{ fontSize: 9, color: "#2a5070" }}>総合スコア</div>
                      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 13, color: rank ? rank.glow : "#00c896", fontWeight: 700 }}>{HEALTH_SCORE} / 100</div>
                    </div>
                  </div>

                  {/* ── 次ランクまでの道 ── */}
                  {rank && nextRank && (
                    <div style={{ background: rank.gradient, border: `1px solid ${rank.border}50`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontSize: 24 }}>{rank.icon}</div>
                          <div style={{ fontSize: 8, color: rank.glow, fontFamily: "'Space Mono',monospace" }}>{rank.label}</div>
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ height: 8, background: "rgba(0,0,0,.4)", borderRadius: 4, marginBottom: 4 }}>
                            <div style={{ width: `${(HEALTH_SCORE / nextScoreThreshold.score) * 100}%`, height: "100%", background: `linear-gradient(90deg,${rank.glow},${nextRank.glow})`, borderRadius: 4, transition: "width 1.5s ease" }} />
                          </div>
                          <div style={{ fontSize: 9, color: rank.glow }}>
                            あと <span style={{ fontFamily: "'Space Mono',monospace", fontWeight: 700 }}>{nextScoreThreshold.score - HEALTH_SCORE}pt</span> で {nextRank.icon} {nextRank.label}へ
                          </div>
                        </div>
                        <div style={{ textAlign: "center", opacity: 0.5 }}>
                          <div style={{ fontSize: 24 }}>{nextRank.icon}</div>
                          <div style={{ fontSize: 8, color: nextRank.glow, fontFamily: "'Space Mono',monospace" }}>{nextRank.label}</div>
                        </div>
                      </div>
                      {/* What to do to level up */}
                      <div style={{ fontSize: 9, color: rank.glow, letterSpacing: 1, marginBottom: 6 }}>ランクアップのヒント</div>
                      {[
                        { icon: "🛒", text: "EC購入をもう1回（+4pt見込み）", axis: "purchase" },
                        { icon: "📊", text: "HRVと水分データも連携する（+6pt見込み）", axis: "data" },
                        { icon: "👥", text: "もう1名紹介する（+5pt見込み）", axis: "referral" },
                      ].map((h, i) => (
                        <div key={i} style={{ display: "flex", gap: 6, marginBottom: 5 }}>
                          <span style={{ fontSize: 11 }}>{h.icon}</span>
                          <span style={{ fontSize: 10, color: "#7090a8", lineHeight: 1.5 }}>{h.text}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {/* ── 現在の特典 ── */}
                  {rank && (
                    <div style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${rank.border}40`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
                      <div style={{ fontSize: 9, color: rank.glow, letterSpacing: 2, marginBottom: 10 }}>現在の特典一覧</div>
                      {rank.perks.map((p, i) => (
                        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 7 }}>
                          <span style={{ color: rank.glow, fontSize: 11 }}>✓</span>
                          <span style={{ fontSize: 11, color: "#90b0c8", lineHeight: 1.5 }}>{p}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {/* ── ランクロードマップ ── */}
                  <div style={{ fontSize: 9, color: "#2a5070", letterSpacing: 2, marginBottom: 10 }}>ランクロードマップ</div>
                  {[...RANKS].reverse().map((r, i) => {
                    const achieved = HEALTH_SCORE >= (SCORE_RANK_THRESHOLDS.find(t => t.rankId === r.id)?.score || 999);
                    const isCurrent = rank?.id === r.id;
                    const threshold = SCORE_RANK_THRESHOLDS.find(t => t.rankId === r.id)?.score || 0;
                    return (
                      <div key={r.id} style={{ display: "flex", gap: 10, marginBottom: 8, opacity: achieved ? 1 : 0.4 }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 20 }}>
                          <div style={{ width: 10, height: 10, borderRadius: "50%", background: achieved ? r.glow : "#1a3a5a", marginTop: 4, boxShadow: isCurrent ? `0 0 8px ${r.glow}` : "none" }} />
                          {i < RANKS.length - 1 && <div style={{ width: 1, flex: 1, background: "#102840", marginTop: 2 }} />}
                        </div>
                        <div style={{ flex: 1, background: isCurrent ? r.gradient : "rgba(255,255,255,.02)", border: `1px solid ${isCurrent ? r.border : "#0e2438"}`, borderRadius: 10, padding: "10px 12px", boxShadow: isCurrent ? `0 0 14px ${r.glow}20` : "none" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            <span style={{ fontSize: 16 }}>{r.icon}</span>
                            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: isCurrent ? r.color : "#4a6a80", fontWeight: 700 }}>{r.label}</div>
                            {isCurrent && <div style={{ fontSize: 8, background: `${r.glow}25`, border: `1px solid ${r.glow}50`, borderRadius: 4, padding: "1px 7px", color: r.glow }}>現在</div>}
                            <div style={{ marginLeft: "auto", fontSize: 9, color: "#2a5070" }}>スコア {threshold}〜</div>
                          </div>
                          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                            {[`割引¥${r.couponDiscount.toLocaleString()}`, `通販${r.ecDiscount}%OFF`, `PT${r.pointRate}%`].map(t => (
                              <div key={t} style={{ fontSize: 8, background: "rgba(0,0,0,.3)", borderRadius: 4, padding: "2px 6px", color: isCurrent ? r.glow : "#2a4060" }}>{t}</div>
                            ))}
                          </div>
                        </div>
                      </div>
